...
unsafe fn public_window_callback_inner(
    ...
    let callback = || match msg {
        ...
        // The active input layout changed (e.g. the user switched keyboard layout/IME, or a
        // layout-switching tool such as Punto Switcher activated one). `LAYOUT_CACHE` is keyed
        // by `HKL`, and Windows can reuse an `HKL` value after the layout it identified is
        // unloaded and a different one is loaded in its place, so re-derive the layout instead
